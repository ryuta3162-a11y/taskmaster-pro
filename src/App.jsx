            ...
            <div className="flex-1 overflow-auto p-4 md:p-10 pt-28 md:pt-32 relative z-0">
                ...
                <div className="max-w-5xl mx-auto animate-fade-in space-y-8">
                       ...
                       <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 flex-1 md:w-56 shadow-inner">
                         ...
                         <div className="flex items-center gap-4">
                            <div className="flex-1 bg-white border border-slate-200 rounded-full h-3 overflow-hidden shadow-inner">
                              <div className="bg-emerald-400 h-full transition-all duration-1000 ease-out" style={{ width: `${requestedTasksProgress}%` }}></div>
                            </div>
                            <span className="text-2xl font-black text-slate-800">{requestedTasksProgress}%</span>
                         </div>
                       </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <button onClick={() => setActiveTab('checklist')} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-lg hover:shadow-2xl hover:border-indigo-300 transition-all group text-center md:text-left relative overflow-hidden">
                      ...
                      {activeTasksCount > 0 && <div className="absolute top-8 right-8 bg-rose-500 text-white text-xs font-black px-4 py-2 rounded-full shadow-lg shadow-rose-200 animate-pulse tracking-widest uppercase">未完了 {activeTasksCount}</div>}
                    </button>
                  </div>
                </div>
              ...
              // === リストチェック画面（★バッジ統合完了） ===
                ...
                <div className="max-w-5xl mx-auto h-full flex flex-col animate-fade-in">
                  ...
                  {/* ★修正: タグフィルターに未完了バッジを追加 */}
                  <div className="flex gap-3 overflow-x-auto pb-6 mb-4 no-scrollbar justify-center md:justify-start">
                    <button onClick={() => setTaskFilter('ALL')} className={`flex-shrink-0 px-6 py-3 rounded-2xl text-sm font-black border-2 transition-all flex items-center gap-2 ${taskFilter === 'ALL' ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                      すべて
                      {activeTasksCount > 0 && <span className={`text-[10px] px-2 py-0.5 rounded-full ${taskFilter === 'ALL' ? 'bg-white text-indigo-600' : 'bg-rose-500 text-white'}`}>{activeTasksCount}</span>}
                    </button>
                    {currentUser?.stores?.map(s => {
                      const storeTaskCount = tasks.filter(t => !t.completed && t.targetTags && t.targetTags.includes(s)).length;
                      return (
                        <button key={s} onClick={() => setTaskFilter(s)} className={`flex-shrink-0 px-6 py-3 rounded-2xl text-sm font-black border-2 transition-all flex items-center gap-2 ${taskFilter === s ? 'bg-indigo-600 text-white border-indigo-600 shadow-md' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
                          {s}
                          {storeTaskCount > 0 && <span className={`text-[10px] px-2 py-0.5 rounded-full ${taskFilter === s ? 'bg-white text-indigo-600' : 'bg-rose-500 text-white'}`}>{storeTaskCount}</span>}
                        </button>
                      );
                    })}
                  </div>
                  
                  {/* ★修正: ステータスタブに未完了バッジを追加 */}
                  <div className="flex bg-slate-100 border border-slate-200 p-2 rounded-2xl mb-8 w-max shadow-inner mx-auto md:mx-0">
                    <button onClick={() => setTaskTab('active')} className={`px-8 py-3 rounded-xl text-sm font-black transition-all flex items-center gap-2 ${taskTab === 'active' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400 hover:text-slate-600'}`}>
                      未完了 <span className="bg-slate-800 text-white text-[10px] px-2 py-0.5 rounded-full">{activeTasksCount}</span>
                    </button>
                    <button onClick={() => setTaskTab('completed')} className={`px-8 py-3 rounded-xl text-sm font-black transition-all flex items-center gap-2 ${taskTab === 'completed' ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400 hover:text-slate-600'}`}>
                      完了済み <span className="bg-slate-300 text-slate-600 text-[10px] px-2 py-0.5 rounded-full">{completedTasksCount}</span>
                    </button>
                  </div>
                      ...
                      <div key={task.id} className="bg-white border rounded-[2.5rem] p-8 md:p-10 flex flex-col md:flex-row gap-8 transition-all duration-700 shadow-sm border-slate-200 hover:shadow-lg animate-fade-in">
                        <div className="flex-1 text-center md:text-left">
                          <div className="flex items-center justify-center md:justify-start gap-3 mb-4 uppercase">
                            ...
                            {/* ★追加: 超過・期限アラートバッジ */}
                            {!task.completed && task.daysRemaining !== null && (
                              <div className="flex items-center">
                                {task.daysRemaining < 0 && (
                                  <div className="flex items-center justify-center px-4 py-1.5 bg-slate-900 text-white border-2 border-slate-900 rounded-xl shadow-md animate-pulse">
                                    <span className="text-[10px] font-black leading-none tracking-widest">⚠ 超過</span>
                                  </div>
                                )}
                                {task.daysRemaining === 0 && (
                                  <div className="flex items-center justify-center px-4 py-1.5 bg-rose-500 text-white border-2 border-rose-600 rounded-xl shadow-[2px_2px_0_0_#e11d48]">
                                    <span className="text-[10px] font-black leading-none tracking-widest">今日まで</span>
                                  </div>
                                )}
                                {task.daysRemaining === 1 && (
                                  <div className="flex items-center justify-center px-4 py-1.5 bg-orange-500 text-white border-2 border-orange-600 rounded-xl shadow-[2px_2px_0_0_#ea580c]">
                                    <span className="text-[10px] font-black leading-none tracking-widest">明日まで</span>
                                  </div>
                                )}
                                {task.daysRemaining === 2 && (
                                  <div className="flex items-center justify-center px-4 py-1.5 bg-amber-400 text-slate-900 border-2 border-amber-500 rounded-xl shadow-[2px_2px_0_0_#f59e0b]">
